...
int check_map(struct bpf_map_info *info) {
  struct bpf_map_info map_expect = {0};
  /* check map info, e.g. datarec is expected size */
  map_expect.key_size = sizeof(__u32);
  map_expect.value_size = sizeof(struct datarec);
  map_expect.max_entries = XDP_ACTION_MAX;
  return check_map_fd_info(info, &map_expect);
}

static void stats_poll(char *pin_dir, int map_fd, __u32 init_id, __u32 map_type,
                       int interval) {
  struct stats_record prev, record = {0};
  struct bpf_map_info info = {0};
  int stats_map_fd, err;
  ...
  while (1) {
    ...
    if (stats_map_fd < 0) {
      return;
    } else if (init_id != info.id) {
      printf("BPF map reloaded\n");
      close(map_fd);
      err = check_map(&info);
      if (err) {
        return;
...
int main(int argc, char **argv) {
  ...
  stats_poll(pin_dir, stats_map_fd, info.id, info.type, interval);
  close(stats_map_fd);
  return EXIT_OK;
}
